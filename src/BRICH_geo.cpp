...
// FIXME: may want to use brich.xml where 32 and 3.25mm are also defined;
...
#define _FIDUCIAL_VOLUME_WIDTH_                                                                    \
  (_HRPPD_TILE_SIZE_ * _HRPPD_MATRIX_DIM_ + _HRPPD_INSTALLATION_GAP_ * (_HRPPD_MATRIX_DIM_ + 1) +  \
   50.0 * mm)
...
static Ref_t createDetector(Detector& desc, xml::Handle_t handle, SensitiveDetector sens) {
  ...
  desc.add(Constant("BRICH_cell_mask", std::to_string(cellMask)));
#ifdef _WITH_IRT_OPTICS_
...
#if defined(_WITH_IRT_OPTICS_) && defined(_WITH_MIRROR_)
  ...
  // Gas volume, aerogel, acrylic filter, mirrors;
  {
      ...
      // Aerogel volume;
      {
...
#ifdef _WITH_IRT_OPTICS_
        {
          ...
          auto surface = new FlatSurface(
              sign * (1 / mm) * TVector3(0, 0, fvOffset + gvOffset + gzOffset), nx, ny);
...
#ifdef _WITH_MIRROR_
    {
      ...
      auto mirrorVis = desc.visAttributes(
          "MirrorSurface_PFRICH"); //mirrorElem.attr<std::string>(_Unicode(vis)));

      // mirror volume, attributes, and placement
      Volume mirrorVol(detName + "_mirror_", mirrorSolid, vesselMaterial);
      mirrorVol.setVisAttributes(mirrorVis);
      auto mirrorPV = gasvolVol.placeVolume(mirrorVol, Position(0, 0, 0));

      // properties
      DetElement mirrorDE(det, "mirror_de_", 0);
      mirrorDE.setPlacement(mirrorPV);
      SkinSurface mirrorSkin(desc, mirrorDE, "mirror_optical_surface_", mirrorSurf, mirrorVol);
      mirrorSkin.isValid();

#ifdef _WITH_IRT_OPTICS_
      msurface  = new CylindricalSurface(sign * (1 / mm) * TVector3(0, 0, fvOffset + gvOffset),
  ...
  // A fake HRPPD container volume, window, photocathode;
  {
...
#ifdef _WITH_IRT_OPTICS_
    {
      ...
      auto surface = new FlatSurface(
          sign * (1 / mm) * TVector3(0, 0, fvzOffset + wzOffset + _HRPPD_WINDOW_THICKNESS_ / 2), nx,
          ny);

      auto radiator = geometry->AddFlatRadiator(
          cdet, "QuartzWindow", CherenkovDetector::Downstream, 0, (G4LogicalVolume*)(0x3), 0,
          /*wndVol, m_FusedSilica,*/ surface, _HRPPD_WINDOW_THICKNESS_ / mm);
    ...
    for (unsigned ix = 0; ix < _HRPPD_MATRIX_DIM_; ix++) {
      ...
      for (unsigned iy = 0; iy < _HRPPD_MATRIX_DIM_; iy++) {
        ...
        TString hrppdName;
        hrppdName.Form("%s-hrppd-%02d%02d", detName.c_str(), ix, iy);
        Volume hrppdVol(hrppdName.Data(), hrppdBox, gasvolMaterial); //vacuumMat);

        // Full size quartz window;
        auto wndBox =
            Box(_HRPPD_TILE_SIZE_ / 2, _HRPPD_TILE_SIZE_ / 2, _HRPPD_WINDOW_THICKNESS_ / 2);
        TString wndName;
        wndName.Form("%s-window-%02d%02d", detName.c_str(), ix, iy);
        Volume wndVol(wndName.Data(), wndBox, windowMaterial);

        auto pcBox = Box(_HRPPD_ACTIVE_AREA_SIZE_ / 2, _HRPPD_ACTIVE_AREA_SIZE_ / 2, pdthick / 2);
        TString pcName;
        pcName.Form("%s-photocathode-%02d%02d", detName.c_str(), ix, iy);
...

        {
          ...
          // Photocathode layer;
          {
            ...
            //printf("@S@: %lu\n", sensorID);
            TString deName;
            deName.Form("%s-sensor-%02d%02d", detName.c_str(), ix, iy);
          ...
          {
            TString absName;
            absName.Form("%s-absorber-%02d%02d", detName.c_str(), ix, iy);
        ...
        vesselVolume.placeVolume(
            hrppdVol, Position(xOffset, yOffset, wzOffset + _HRPPD_CONTAINER_VOLUME_HEIGHT_ / 2));

#ifdef _WITH_IRT_OPTICS_
        {
          // Photocathode surface;
          auto surface = new FlatSurface(
              (1 / mm) * TVector3(sign * xOffset, yOffset, sign * (fvzOffset + zpdc)),
              TVector3(1 * sign, 0, 0), TVector3(0, -1, 0));

          {
            // '0': BRICH has no division in sectors (unlike e.g. dRICH);
            unsigned sector = 0;
            ...
            for (unsigned iq = 0; iq < 1; iq++) {
              ...
              if (cdet->m_OpticalBoundaries[CherenkovDetector::Downstream].find(sector) !=
                ...
                for (auto boundary :
                     cdet->m_OpticalBoundaries[CherenkovDetector::Downstream][sector])
                  irt->AddOpticalBoundary(boundary);
...
DECLARE_DETELEMENT(epic_BRICH, createDetector)
