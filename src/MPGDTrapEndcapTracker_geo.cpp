...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens) {
  ...
  map<string, Volume> modules_overlap_N, modules_overlap_S;
  map<string, Placements> sensitives_N, sensitives_S;
  map<string, Placements> sensitives_overlap_N, sensitives_overlap_S;
  map<string, std::vector<VolPlane>> volplane_surfaces_N, volplane_surfaces_S;
  map<string, std::array<double, 2>> module_thicknesses;
  PlacedVolume pv_N, pv_S, pv;
  PlacedVolume pv_overlap_N, pv_overlap_S; //pv_overlap to automatize some parts
  ...
  for (xml_coll_t mi(x_det, _U(module)); mi; ++mi, ++m_id) {
    ...
    for (ci.reset(), n_sensor = 1, c_id = 0, posY = -y; ci; ++ci, ++c_id) {
      ...
      Material f_mat =
          description.material(getAttrOrDefault(c, _Unicode(materialf), c.materialStr()));

      // names
      string c_name         = _toString(c_id, "component%d");
      string c_name_overlap = c_name + "_overlap";

      string fname_in  = _toString(c_id, "component%d_frame_in");
      string fname_out = _toString(c_id, "component%d_frame_out");

      string f_name_overlap_in  = fname_in + "_overlap";
      string f_name_overlap_out = fname_out + "_overlap";
      ...
      Volume c_vol_overlap_N(_toString(c_id, "%d_overlap_N"), comp_o_N, c_mat);
      Volume c_vol_overlap_S(_toString(c_id, "%d_overlap_S"), comp_o_S, c_mat);
      ...
      c_vol_overlap_N.setVisAttributes(description.visAttributes(c.visStr()));
      c_vol_overlap_S.setVisAttributes(description.visAttributes(c.visStr()));
      ...
      pv_overlap_N = m_volume_overlap_N.placeVolume(
          c_vol_overlap_N, Position(0, posY + c_thick / 2, comp_pos_Zb_N - zbox_N));
      pv_overlap_S = m_volume_overlap_S.placeVolume(
          c_vol_overlap_S, Position(0, posY + c_thick / 2, comp_pos_Zb_S - zbox_S));
      ...
      if (c.isSensitive()) {
        ...
        pv_overlap_N.addPhysVolID("sensor", n_sensor);
        pv_overlap_S.addPhysVolID("sensor", n_sensor);

        c_vol_N.setSensitiveDetector(sens);
        c_vol_S.setSensitiveDetector(sens);
        c_vol_overlap_N.setSensitiveDetector(sens);
        c_vol_overlap_S.setSensitiveDetector(sens);

        sensitives_N[m_nam].push_back(pv_N);
        sensitives_S[m_nam].push_back(pv_S);
        sensitives_overlap_N[m_nam].push_back(pv_overlap_N);
        sensitives_overlap_S[m_nam].push_back(pv_overlap_S);
        ...
        VolPlane surf_N(c_vol_N, type, inner_thickness, outer_thickness, u, v, n);
        VolPlane surf_S(c_vol_S, type, inner_thickness, outer_thickness, u, v, n);
        VolPlane surf_overlap_N(c_vol_overlap_N, type, inner_thickness, outer_thickness, u, v, n);
        VolPlane surf_overlap_S(c_vol_overlap_S, type, inner_thickness, outer_thickness, u, v, n);
        volplane_surfaces_N[m_nam].push_back(surf_N);
        volplane_surfaces_S[m_nam].push_back(surf_S);
        volplane_surfaces_N[m_nam].push_back(surf_overlap_N);
        volplane_surfaces_S[m_nam].push_back(surf_overlap_S);
        //--------------------------------------------
      }
      ...
      if (has_frame) {
        ...
        pv_N = m_volume_N.placeVolume(f_vol_N_in,
                                      Position(0, posY + c_thick / 2, frame_pos_Z_N_in - zN));
        pv_N = m_volume_N.placeVolume(f_vol_N_out,
                                      Position(0, posY + c_thick / 2, frame_pos_Z_N_out - zN));
        pv_S = m_volume_S.placeVolume(f_vol_S_in,
                                      Position(0, posY + c_thick / 2, frame_pos_Z_S_in - zS));
        pv_S = m_volume_S.placeVolume(f_vol_S_out,
                                      Position(0, posY + c_thick / 2, frame_pos_Z_S_out - zS));
        ...
        pv_overlap_N = m_volume_overlap_N.placeVolume(
            f_vol_overlap_N_in, Position(0, posY + c_thick / 2, frame_pos_Zb_N_in - zbox_N));
        pv_overlap_N = m_volume_overlap_N.placeVolume(
            f_vol_overlap_N_out, Position(0, posY + c_thick / 2, frame_pos_Zb_N_out - zbox_N));
        pv_overlap_S = m_volume_overlap_S.placeVolume(
            f_vol_overlap_S_in, Position(0, posY + c_thick / 2, frame_pos_Zb_S_in - zbox_S));
        pv_overlap_S = m_volume_overlap_S.placeVolume(
            f_vol_overlap_S_out, Position(0, posY + c_thick / 2, frame_pos_Zb_S_out - zbox_S));
      }

      posY += c_thick;
      thickness_so_far += c_thick;
    }
    modules_N[m_nam]         = m_volume_N;
    modules_S[m_nam]         = m_volume_S;
    modules_overlap_N[m_nam] = m_volume_overlap_N;
    modules_overlap_S[m_nam] = m_volume_overlap_S;
  }
  ...
  for (xml_coll_t li(x_det, _U(layer)); li; ++li) {
    ...
    for (xml_coll_t ri(x_layer, _U(ring)); ri; ++ri) {
      ...
      double dz_offset             = getAttrOrDefault(x_ring, _Unicode(dz_offset), 0.0);
      int nmodules                 = x_ring.nmodules();
      int nextOverlapModuleId      = nmodules;
      int nmodules_per_quadrant    = (int)(nmodules / 4);
      string m_nam                 = x_ring.moduleStr();
      Volume m_vol                 = modules_S[m_nam];
      Volume m_vol_overlap         = modules_overlap_S[m_nam];
      ...
      for (int k = 0; k < nmodules; ++k) {
        ...
        double dz_final = dz;
        if (k < nmodules_per_quadrant ||
            (k >= 2 * nmodules_per_quadrant && k < 3 * nmodules_per_quadrant))
          dz_final = dz + dz_offset;
        ...
        if (!reflect) {
          ...
          pv = layer_vol.placeVolume(m_vol, mod_num,
                                     Transform3D(RotationZYX(0, -M_PI / 2 - phi, -M_PI / 2),
        ...
        if (k % nmodules_per_quadrant == 0 ||
            k % (nmodules_per_quadrant) == nmodules_per_quadrant - 1) {
          const int overlapModuleId = nextOverlapModuleId++;
          //if(-1 == 1){
          double xo = 0, yo = 0, roto = 0;

          if (k == 0)
            xo = -R, yo = xb, roto = -M_PI / 2;
          else if (k == nmodules_per_quadrant - 1)
            xo = xb, yo = -R, roto = M_PI;
          else if (k == nmodules_per_quadrant)
            xo = -xb, yo = -R, roto = M_PI;
          else if (k == 2 * nmodules_per_quadrant - 1)
            xo = R, yo = xb, roto = M_PI / 2;
          else if (k == 2 * nmodules_per_quadrant)
            xo = R, yo = -xb, roto = M_PI / 2;
          else if (k == 3 * nmodules_per_quadrant - 1)
            xo = -xb, yo = R, roto = 0;
          else if (k == 3 * nmodules_per_quadrant)
            xo = xb, yo = R, roto = 0;
          else if (k == 4 * nmodules_per_quadrant - 1)
            xo = -R, yo = -xb, roto = -M_PI / 2;

          if (!reflect) {
            DetElement module(layer_element, m_base + "_overlap_pos", det_id);
            pv = layer_vol.placeVolume(
                m_vol_overlap, overlapModuleId,
                Transform3D(RotationZYX(0, roto, -M_PI / 2), Position(xo, yo, zstart + dz_final)));
