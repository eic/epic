  ...
  if (det_handle.hasChild(_Unicode(wedge_box))) {
    ...
    for (; sector < sectors_handle.number(); sector++, sector_phi += sectors_handle.deltaphi()) {
      for (int side = -1; side <= 1; side += 2) {
        envelope_v.placeVolume(wedge_box_side_v,
    ...
    for (; row < rows_handle.number(); row++, row_phi += rows_handle.deltaphi()) {
      ...
      for (xml_coll_t family_handle{rows_handle, _Unicode(family)}; family_handle; ++family_handle) {
        ...
        for (unsigned int tower = 0; tower < number; tower++, tower_id += dir_sign) {
          ...
          const string t_name =
              _toString(sector, "sector%d") + _toString(row, "_row%d") + _toString(tower_id, "_tower%d");
          envelope_v
              .placeVolume(Volume{t_name, Trap{z, theta, phi, y1, x1, x2, alpha1, y2, x3, x4, alpha2},
          ...
          if (sectors_handle.hasChild(_Unicode(carbon_fiber_support))) {
            ...
            for (int side = (row == 0) ? 0 : 1; side <= 1; side++) {
              envelope_v
                  .placeVolume(Volume{"fiber_structure_longitudinal", trap_long, carbon_fiber_support_mat},
                               Transform3D{RotationZ{-M_PI_2 + sector_phi + row_phi +
                                                     (2 * side - 1) * rows_handle.deltaphi() / 2}} *
                                   Transform3D{Position{0. * cm, row_rmin, dir_sign * dz}} *
                                   Transform3D{RotationX{-M_PI / 2 + dir_sign * beta}} *
            ...
            const double dz_trans =
                dz_prev + (tower_gap_longitudinal / 2 + carbon_fiber_support_handle.thickness() / 2) /
