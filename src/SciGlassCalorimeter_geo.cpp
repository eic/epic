...
static Ref_t create_detector(Detector& desc, xml::Handle_t handle, SensitiveDetector sens)
{
    ...
    auto mod_phi_projectivity_tilt = 
        getAttrOrDefault<float>(x_mod, _Unicode(phi_projectivity_tilt), 0.0);
    auto mod_eta_projectivity_offset = 
        getAttrOrDefault<float>(x_mod, _Unicode(eta_projectivity_offset), 0.0);
    printout(DEBUG, "SciGlassCalorimeter", "Proj: phi = %f, eta = %f", mod_phi_projectivity_tilt, mod_eta_projectivity_offset);
    ...
    for (int k_eta = 0; k_eta < std::max(n_eta_neg, n_eta_pos); k_eta++) {
        ...
        const auto mod_x1 = getAttrOrDefault<float>(x_mod, _Unicode(x1),
                            xcrd_dphi * cos(mod_phi_projectivity_tilt));
        const auto mod_y1 = getAttrOrDefault<float>(x_mod, _Unicode(y1),
                            mod_x1);
        ...
        auto mod_x2 = getAttrOrDefault<float>(x_mod, _Unicode(x2),
                      mod_x1 * (1 + mod_expansion * mod_length));
        auto mod_y2 = getAttrOrDefault<float>(x_mod, _Unicode(y2),
                      mod_y1 * (1 + mod_expansion * mod_length));
        ...
        for (size_t j_phi = 0; j_phi < n_phi; j_phi++) {
            ...
            if (k_eta < n_eta_neg) {
                Transform3D tr_neg = Translation3D(x, y, - det_pos.z() + mod_eta_projectivity_offset - z)
                                   * RotationZ(phi + mod_phi_projectivity_tilt)
                                   * RotationY(M_PI_2 + theta);
    ...
    Transform3D tr = Translation3D(det_pos.x(), det_pos.y(), det_pos.z())
                   * RotationZYX(det_rot.z(), det_rot.y(), det_rot.x());
