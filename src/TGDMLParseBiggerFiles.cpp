...
When most solids or volumes are added to the geometry they
 ...
 element process then needs to be linked thru the function TGDMLParseBiggerFiles
...
//ClassImp(TGDMLParseBiggerFiles);

////////////////////////////////////////////////////////////////////////////////
/// Constructor
/*
TGDMLParseBiggerFiles::TGDMLParseBiggerFiles()
{
...
TGeoVolume* TGDMLParseBiggerFiles::GDMLReadFile(const char* filename) {
  // First create engine
  TXMLEngine* gdml = new TXMLEngine;
  gdml->SetSkipComments(kTRUE);

  // Now try to parse xml file
  //maxbuf is important as it allows us to extend the allowed buffer size
  //dont set it too big though as that might lead to memory overflows
  Int_t maxbuf            = 50000000;
  XMLDocPointer_t gdmldoc = gdml->ParseFile(filename, maxbuf);
  if (gdmldoc == nullptr) {
    delete gdml;
    return nullptr;
  } else {
  ...
  if (fNunitless && gGeoManager->GetDefaultUnits() == TGeoManager::kRootUnits) {
    Warning(
        ...
        "units are currently ROOT units [cm, deg]. This can cause unexpected behaviour with "
        "respect "
        "to the GDML schema. To remove this warning, either use explicit units or call the static "
        "method "
...
const char* TGDMLParseBiggerFiles::ParseGDML(TXMLEngine* gdml, XMLNodePointer_t node) {
  DefineConstants();
  ...
  //*********************************************
    ...
    // CHECK FOR TAGS NOT SUPPORTED
             ...
             ((strcmp(name, "isotope")) != 0) && ((strcmp(name, "triangular")) != 0) &&
             ((strcmp(name, "quadrangular")) != 0)) {
    std::cout << "Error: Unsupported GDML Tag Used :" << name << ". Please Check Geometry/Schema."
              << std::endl;
  }
...
double TGDMLParseBiggerFiles::Evaluate(const char* evalline) {
...
Int_t TGDMLParseBiggerFiles::SetAxis(const char* axisString) {
  Int_t axis = 0;
...
const char* TGDMLParseBiggerFiles::NameShort(const char* name) {
...
XMLNodePointer_t TGDMLParseBiggerFiles::ConProcess(TXMLEngine* gdml, XMLNodePointer_t node,
...
void TGDMLParseBiggerFiles::DefineConstants() {
...
XMLNodePointer_t TGDMLParseBiggerFiles::QuantityProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                        XMLAttrPointer_t attr) {
  TString name  = "";
  TString value = "";
  TString unit  = "1.0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::MatrixProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                      XMLAttrPointer_t attr) {
  TString name = "";
  Int_t coldim = 1;
  std::string values;
...
///
XMLNodePointer_t TGDMLParseBiggerFiles::OpticalSurfaceProcess(TXMLEngine* gdml,
    ...
    if ((strcmp(gdml->GetNodeName(child), "property")) == 0) {
      while (attr != nullptr) {
        ...
        if (tempattr == "name") {
          ...
          if (!matrix)
            Error("OpticalSurfaceProcess", "Reference matrix %s for optical surface %s not found",
                  ref.Data(), name.Data());
...
TString TGDMLParseBiggerFiles::GetScale(const char* unit) {
  TString retunit = "";
...
Double_t TGDMLParseBiggerFiles::GetScaleVal(const char* sunit) {
  auto def_units   = gGeoManager->GetDefaultUnits();
  Double_t retunit = 0.;
  TString unit(sunit);
  unit.ToLower();
...
Double_t TGDMLParseBiggerFiles::Value(const char* svalue) const {
  char* end;
...
XMLNodePointer_t TGDMLParseBiggerFiles::PosProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLAttrPointer_t attr) {
...
XMLNodePointer_t TGDMLParseBiggerFiles::RotProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLAttrPointer_t attr) {
...
XMLNodePointer_t TGDMLParseBiggerFiles::SclProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLAttrPointer_t attr) {
  TString xpos = "0";
  TString ypos = "0";
  TString zpos = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::IsoProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLAttrPointer_t parentn) {
  TString z    = "0";
  TString name = "";
  TString n    = "0";
  TString atom = "0";
  ...
  if (!iso) {
    ...
    Info("TGDMLParseBiggerFiles", "Re-use existing isotope: %s", iso->GetName());
  }
...
XMLNodePointer_t TGDMLParseBiggerFiles::EleProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLNodePointer_t parentn, Bool_t hasIsotopes,
                                                   Bool_t hasIsotopesExtended)

{
  ...
  if (!ele) {
    ...
    Info("TGDMLParseBiggerFiles", "Re-use existing element: %s", ele->GetName());
  }
  felemap[local_name.Data()] = ele;
  return node;
}
...
XMLNodePointer_t TGDMLParseBiggerFiles::MatProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLAttrPointer_t attr, int z) {
  ...
  if (z == 1) {
    ...
    if (!mat) {
      ...
      Info("TGDMLParseBiggerFiles", "Re-use existing material: %s", mat->GetName());
    }
    ...
    if (!mat_ele) {
      ...
      Info("TGDMLParseBiggerFiles", "Re-use existing material-element: %s", mat_ele->GetName());
    }
    ...
    if (!mat) {
      ...
      mix = (TGeoMixture*)mat;
      if (gDebug >= 2)
        Info("TGDMLParseBiggerFiles", "Re-use existing material-mixture: %s", mix->GetName());
    } else {
      Fatal("TGDMLParseBiggerFiles",
  ...
  if (!med) {
    ...
    Info("TGDMLParseBiggerFiles", "Re-use existing medium: %s", med->GetName());
  }
...
XMLNodePointer_t TGDMLParseBiggerFiles::SkinSurfaceProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                                           XMLAttrPointer_t attr) {
...
XMLNodePointer_t TGDMLParseBiggerFiles::BorderSurfaceProcess(TXMLEngine* gdml,
                                                             XMLNodePointer_t node,
                                                             XMLAttrPointer_t attr) {
  ...
  if (!node1 || !node2)
    Fatal("BorderSurfaceProcess", "Border surface %s: not found nodes %s [%s] or %s [%s]",
          name.Data(), nodename[0].Data(), node1 ? "present" : "missing", nodename[1].Data(),
          node2 ? "present" : "missing");
...
TGeoTranslation* TGDMLParseBiggerFiles::GetPosition(const char* name) {
...
TGeoRotation* TGDMLParseBiggerFiles::GetRotation(const char* name) {
...
TGeoScale* TGDMLParseBiggerFiles::GetScaleObj(const char* name) {
...
TGeoShape* TGDMLParseBiggerFiles::GetSolid(const char* name) {
...
TGeoVolume* TGDMLParseBiggerFiles::GetVolume(const char* name) {
  // Get defined solid by name.
...
XMLNodePointer_t TGDMLParseBiggerFiles::VolProcess(TXMLEngine* gdml, XMLNodePointer_t node) {
  XMLAttrPointer_t attr;
...
XMLNodePointer_t TGDMLParseBiggerFiles::BooSolid(TXMLEngine* gdml, XMLNodePointer_t node,
                                                 XMLAttrPointer_t attr, int num) {
  TString reftemp        = "";
  TString tempattr       = "";
  XMLNodePointer_t child = gdml->GetChild(node);
  ...
  switch (num) {
  case 1:
    boolean = new TGeoCompositeShape(NameShort(name),
                                     new TGeoSubtraction(first, second, firstMatrix, secondMatrix));
    break; // SUBTRACTION
  case 2:
    boolean = new TGeoCompositeShape(
        NameShort(name), new TGeoIntersection(first, second, firstMatrix, secondMatrix));
    break; // INTERSECTION
  case 3:
    boolean = new TGeoCompositeShape(NameShort(name),
...
XMLNodePointer_t TGDMLParseBiggerFiles::UsrProcess(TXMLEngine* gdml, XMLNodePointer_t node) {
  XMLNodePointer_t child = gdml->GetChild(node);
...
XMLNodePointer_t TGDMLParseBiggerFiles::AssProcess(TXMLEngine* gdml, XMLNodePointer_t node) {
  TString name       = gdml->GetAttr(node, "name");
  TString local_name = name;
  TString reftemp    = "";
...
XMLNodePointer_t TGDMLParseBiggerFiles::TopProcess(TXMLEngine* gdml, XMLNodePointer_t node) {
...
XMLNodePointer_t TGDMLParseBiggerFiles::Box(TXMLEngine* gdml, XMLNodePointer_t node,
                                            XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString xpos    = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::Ellipsoid(TXMLEngine* gdml, XMLNodePointer_t node,
                                                  XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString ax      = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::ElCone(TXMLEngine* gdml, XMLNodePointer_t node,
                                               XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString dx      = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::Paraboloid(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString rlopos  = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::Arb8(TXMLEngine* gdml, XMLNodePointer_t node,
                                             XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString v1xpos  = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::Tube(TXMLEngine* gdml, XMLNodePointer_t node,
                                             XMLAttrPointer_t attr) {
  TString lunit    = fDefault_lunit.c_str();
  TString aunit    = fDefault_aunit.c_str();
  bool unitless_l  = true;
...
XMLNodePointer_t TGDMLParseBiggerFiles::CutTube(TXMLEngine* gdml, XMLNodePointer_t node,
                                                XMLAttrPointer_t attr) {
  TString lunit    = fDefault_lunit.c_str();
  TString aunit    = fDefault_aunit.c_str();
  bool unitless_l  = true;
  ...
  TGeoCtub* cuttube =
      new TGeoCtub(NameShort(name), rminline, rmaxline, zline / 2, startphiline, deltaphiline,
...
XMLNodePointer_t TGDMLParseBiggerFiles::Cone(TXMLEngine* gdml, XMLNodePointer_t node,
                                             XMLAttrPointer_t attr) {
  TString lunit    = fDefault_lunit.c_str();
  TString aunit    = fDefault_aunit.c_str();
  bool unitless_l  = true;
  ...
  if (dphi < 360.)
    cone = new TGeoConeSeg(NameShort(name), zline / 2, rmin1line, rmax1line, rmin2line, rmax2line,
                           sphi, ephi);
  else
...
XMLNodePointer_t TGDMLParseBiggerFiles::Trap(TXMLEngine* gdml, XMLNodePointer_t node,
                                             XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  TString aunit   = fDefault_aunit.c_str();
  bool unitless_l = true;
  ...
  TGeoTrap* trap =
      new TGeoTrap(NameShort(name), zline / 2, thetaline, philine, y1line / 2, x1line / 2,
...
XMLNodePointer_t TGDMLParseBiggerFiles::Trd(TXMLEngine* gdml, XMLNodePointer_t node,
                                            XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString x1      = "0";
  ...
  TGeoTrd2* trd =
      new TGeoTrd2(NameShort(name), x1line / 2, x2line / 2, y1line / 2, y2line / 2, zline / 2);
...
XMLNodePointer_t TGDMLParseBiggerFiles::Polycone(TXMLEngine* gdml, XMLNodePointer_t node,
                                                 XMLAttrPointer_t attr) {
  TString lunit    = fDefault_lunit.c_str();
  TString aunit    = fDefault_aunit.c_str();
  bool unitless_l  = true;
...
XMLNodePointer_t TGDMLParseBiggerFiles::Polyhedra(TXMLEngine* gdml, XMLNodePointer_t node,
                                                  XMLAttrPointer_t attr) {
  TString lunit    = fDefault_lunit.c_str();
  TString aunit    = fDefault_aunit.c_str();
  bool unitless_l  = true;
  ...
  TGeoPgon* polyg =
      new TGeoPgon(NameShort(name), startphiline, deltaphiline, numsidesline, numplanes);
  Int_t zno = 0;
...
XMLNodePointer_t TGDMLParseBiggerFiles::Sphere(TXMLEngine* gdml, XMLNodePointer_t node,
                                               XMLAttrPointer_t attr) {
  TString lunit      = fDefault_lunit.c_str();
  TString aunit      = fDefault_aunit.c_str();
  bool unitless_l    = true;
  ...
  TGeoSphere* sphere = new TGeoSphere(NameShort(name), rminline, rmaxline, startthetaline,
                                      deltathetaline, startphiline, deltaphiline);
...
XMLNodePointer_t TGDMLParseBiggerFiles::Torus(TXMLEngine* gdml, XMLNodePointer_t node,
                                              XMLAttrPointer_t attr) {
  TString lunit    = fDefault_lunit.c_str();
  TString aunit    = fDefault_aunit.c_str();
  bool unitless_l  = true;
  ...
  TGeoTorus* torus =
      new TGeoTorus(NameShort(name), rtorline, rminline, rmaxline, startphiline, deltaphiline);
...
XMLNodePointer_t TGDMLParseBiggerFiles::Hype(TXMLEngine* gdml, XMLNodePointer_t node,
                                             XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  TString aunit   = fDefault_aunit.c_str();
  bool unitless_l = true;
  ...
  TGeoHype* hype =
      new TGeoHype(NameShort(name), rminline, instline, rmaxline, outstline, zline / 2);
...
XMLNodePointer_t TGDMLParseBiggerFiles::Para(TXMLEngine* gdml, XMLNodePointer_t node,
                                             XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  TString aunit   = fDefault_aunit.c_str();
  bool unitless_l = true;
  ...
  TGeoPara* para =
      new TGeoPara(NameShort(name), xline / 2, yline / 2, zline / 2, alphaline, thetaline, philine);
...
XMLNodePointer_t TGDMLParseBiggerFiles::TwistTrap(TXMLEngine* gdml, XMLNodePointer_t node,
                                                  XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  TString aunit   = fDefault_aunit.c_str();
  bool unitless_l = true;
  ...
  TGeoGtra* twtrap = new TGeoGtra(NameShort(name), zline / 2, thetaline, philine, twistline,
                                  y1line / 2, x1line / 2, x2line / 2, alpha1line, y2line / 2,
                                  x3line / 2, x4line / 2, alpha2line);
...
XMLNodePointer_t TGDMLParseBiggerFiles::ElTube(TXMLEngine* gdml, XMLNodePointer_t node,
                                               XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString xpos    = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::Orb(TXMLEngine* gdml, XMLNodePointer_t node,
                                            XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString r       = "0";
...
XMLNodePointer_t TGDMLParseBiggerFiles::Xtru(TXMLEngine* gdml, XMLNodePointer_t node,
                                             XMLAttrPointer_t attr) {
  TString lunit   = fDefault_lunit.c_str();
  bool unitless_l = true;
  TString x       = "0";
  ...
  for (int j = 0; j < sect; j++) {
    xtru->DefineSection((int)section[j][0], section[j][1], section[j][2], section[j][3],
                        section[j][4]);
  }
...
XMLNodePointer_t TGDMLParseBiggerFiles::Tessellated(TXMLEngine* gdml, XMLNodePointer_t node,
                                                    XMLAttrPointer_t attr) {
...
XMLNodePointer_t TGDMLParseBiggerFiles::ScaledSolid(TXMLEngine* gdml, XMLNodePointer_t node,
                                                    XMLAttrPointer_t attr) {
...
XMLNodePointer_t TGDMLParseBiggerFiles::Reflection(TXMLEngine* gdml, XMLNodePointer_t node,
                                                   XMLAttrPointer_t attr) {
  std::cout << "WARNING! The reflectedSolid is obsolete! Use scale transformation instead!"
            << std::endl;
...
This class is a helper class for TGDMLParseBiggerFiles.   It assists in the
...
TGeoMatrix* TGDMLRefl::GetMatrix() { return fMatrix; }
