...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector /* sens */) {
  ...
  for (uint pipeN = 0; pipeN < xCenters.size(); pipeN++) {
    // beam pipe matter
    ConeSegment s_tube(lengths[pipeN] / 2.0,        // length
    ...
    // beam pipe coating
    ConeSegment s_coat(lengths[pipeN] / 2.0,                  // length
    ...
    // beam pipe vacuum
    ConeSegment s_vacm(lengths[pipeN] / 2.0,                  // length
                       0,                                     // r2 in
    ...
    assembly.placeVolume(v_coat, Transform3D(RotationY(thetas[pipeN]),
                                             Position(xCenters[pipeN], 0, zCenters[pipeN])));
    assembly.placeVolume(v_vacm, Transform3D(RotationY(thetas[pipeN]),
                                             Position(xCenters[pipeN], 0, zCenters[pipeN])));
  }
