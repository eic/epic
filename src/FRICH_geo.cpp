...
static Ref_t createDetector(Detector& desc, xml::Handle_t handle, SensitiveDetector sens) {
  ...
  // Gas volume, aerogel, acrylic filter, mirrors;
  {
...

    {
      ...
      IntersectionSolid mirrorSolid(mirrorFvSolid, mirrorSphere,
                                    Position(0, 0, mifvLength / 2 - miRadius));
      ...
      auto mirrorPV =
          gasvolVol.placeVolume(mirrorVol, Position(0, 0, gvLength / 2 - mifvLength / 2));
      ...
      msurface = new SphericalSurface(
          sign * (1 / mm) * TVector3(0, 0, fvOffset + gvOffset + gvLength / 2 - miRadius),
          (1 / mm) * miRadius);
  ...
  // A fake HRPPD container volume, window, photocathode;
  {
...
#ifdef _WITH_IRT_OPTICS_
    {
      ...
      auto surface = new FlatSurface(
          sign * (1 / mm) * TVector3(0, 0, fvzOffset + _wzOffset - _HRPPD_WINDOW_THICKNESS_ / 2),
          nx, ny);
      ...
      for (unsigned iy = 0; iy < _HRPPD_MATRIX_DIM_; iy++) {
...

        {
          ...
          Transform3D transform(
              rot, Position(xOffset, yOffset, _wzOffset - _HRPPD_CONTAINER_VOLUME_HEIGHT_ / 2));
...
#ifdef _WITH_IRT_OPTICS_
        {
          ...
          auto surface = new FlatSurface(
              (1 / mm) * TVector3(sign * xOffset, yOffset, sign * (fvzOffset + _zpdc)),
