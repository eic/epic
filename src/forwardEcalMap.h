...
public:
  ...
  double blockLength() { return mBlockLength; }
  double spaceBetweenBlock() { return mSpaceBetweenBlock; }
  double backPlateZ() { return mBackPlateZ; }
  ...
  double offsetXBeamPipe(int ns) { return mOffsetXBeamPipe[ns]; }
  ...
  double doesBlockExist(int ns, int row, int col) {
    if (0 <= row && row < mMaxRowBlock && 0 <= col && col < mNColBlock[ns][row])
      return 1;
    return 0;
  }
  ...
  int blockIdFromTower(int ns, int rowT, int colT) {
    return blockId(ns, rowBlock(rowT), colBlock(colT));
  }
  double xTower(int ns, int row, int col) {
    return xBlock(ns, rowBlock(row), colBlock(col)) +
           (1 - 2 * ns) * (col % mNTowerInBlock - 1.5) * towerSize();
  }
  double yTower(int ns, int row) {
    return yBlock(ns, rowBlock(row)) - (row % mNTowerInBlock - 1.5) * towerSize();
  }
  double doesTowerExist(int ns, int row, int col) {
    return doesBlockExist(ns, rowBlock(row), colBlock(col));
  }
  ...
  double xFeebd(int ns, int row, int col) {
    return xBlock(ns, row, col * 2) + (0.5 - ns) * blockSize();
  }
  ...
  int powerGroup(int rowB, int colB) {
    ...
    } else if (rowB >= 6 && rowB <= 14) {
      return rowB - 2;
    } else if (rowB >= 15 && rowB <= 17) {
      if (colB < 18) {
        return rowB - 2;
      } else {
        return rowB - 10;
      }
    } else if (rowB >= 18 && rowB <= 20) {
      return rowB - 2;
    } else if (rowB >= 21 && rowB <= 23) {
      if (colB < 18) {
        return rowB - 2;
      } else {
        return rowB + 6;
      }
    } else if (rowB >= 24 && rowB <= 32) {
      return rowB - 2;
    } else if (rowB == 33) {
      if (colB < 2) {
        return 30;
  ...
  int powerGroupColor(int pg) {
    int pgc[mNPowerGroup] = {kOrange,      kGreen - 9, kViolet - 9,  kCyan - 9,    kGreen + 2,
                             kAzure + 7,   kViolet,    kMagenta - 9, kGreen - 9,   kViolet - 9,
                             kCyan - 9,    kGray,      kOrange,      kMagenta - 9, kGreen - 9,
                             kViolet - 9,  kCyan - 9,  kViolet - 9,  kGreen - 9,   kMagenta - 9,
                             kOrange,      kGray,      kCyan - 9,    kViolet - 9,  kGreen - 9,
                             kMagenta - 9, kViolet,    kAzure + 7,   kGreen + 2,   kCyan - 9,
                             kViolet - 9,  kGreen - 9, kOrange};
    return pgc[pg - 1];
  }
  ...
  void decodeMcHit(ULong_t id, int& system, int& ns, int& row, int& col, int& x, int& y) {
    system = id & 0xff;
    ns     = (id >> 8) & 0x1;
    row    = (id >> 9) & 0x3f;
    col    = (id >> 15) & 0x1f;
  ...
  void decodeMcHit2(ULong_t id, int& system, int& ns, int& row, int& col, int& tx, int& ty,
                    int& fiberx, int& fibery, int& x, int& y) {
    system = id & 0xff;
    ns     = (id >> 8) & 0x1;
    row    = (id >> 9) & 0x3f;
    col    = (id >> 15) & 0x1f;
  ...
  void mcHitToTowerRowCol(ULong_t id, int& ns, int& trow, int& tcol) {
    ...
    trow = row * mNTowerInBlock + (3 - y);
    if (ns == 0) {
      if (row >= 18 && row <= 20) {
        tcol = col * mNTowerInBlock + x + 3;
      } else {
        tcol = col * mNTowerInBlock + x;
      }
    } else {
      if (row >= 18 && row <= 20) {
        tcol = col * mNTowerInBlock + (3 - x) + 8;
      } else {
        tcol = col * mNTowerInBlock + (3 - x);
      }
    }
  }
};
