...
#define DEBUG_MPGDCylinderBarrelTracker
  ...
  if (modules.size()!=1) {
    ...
    printout(ERROR,"MPGDCylinderBarrelTracker",
	     "Number of modules = %d. Must be = 1",(int)modules.size());
  ...
  typedef struct{ string name; double rmin; int nphi; double offset; int io; } StaveModel;
  StaveModel staveModels[4]; int sector2Models[2][2];
  xml_coll_t mi(x_mod,_U(model));
  if (mi.size()!=2) {
    printout(ERROR,"MPGDCylinderBarrelTracker",
	     "Number of models = %d. Must be = 2",(int)mi.size());
    throw runtime_error("Logics error in building modules.");
  }
  int nStaveModels; for (nStaveModels = 0; mi; ++mi) {
    ...
    int nphi = int(2*M_PI*rmin1/stave_width+.5), nphi2 = int(2*M_PI*rmin2/stave_width+.5);
    if (nphi2!=nphi) {
      printout(ERROR,"MPGDCylinderBarrelTracker",
    ...
    staveModel.rmin = rmin1; staveModel.nphi = nphi; staveModel.offset = offset;
    int io; if (nStaveModels==1) // 1st encountered "x_model" => ...
    ...
    if (fabs(rmin2-rmin1)>1.e6) {
      ...
      staveMode2.rmin = rmin2; staveModel.nphi = nphi; staveMode2.offset = offset;
      staveMode2.io = io;
  ...
  for (int iSM = 0; iSM<nStaveModels; iSM++) {
    ...
    printout(DEBUG,"MPGDCylinderBarrelTracker",
	     "Stave Model #%d \"%s\": rmin = %.2f cm offset = ±%.2f cm %s",
	     iSM,sM.name.c_str(),sM.rmin,sM.offset/2,sM.io?"Outer":"Inner");
  }
  ...
  typedef struct{
    string name; double width; string material; string vis; } Frame;
  Frame frames[2];
  xml_coll_t fi(x_mod,_U(frame));
  if (fi.size()!=2) {
    printout(ERROR,"MPGDCylinderBarrelTracker",
	     "Number of frames = %d. Must be = 2",(int)fi.size());
    throw runtime_error("Logics error in building modules.");
  }
  printout(DEBUG,"MPGDCylinderBarrelTracker","2 Frames:");
  int iFr; for (iFr = 0; fi; ++fi, iFr++) {
    xml_comp_t x_frame = fi;
    string name = x_frame.nameStr(); double width = x_frame.width();
    string material = x_frame.materialStr(), vis = x_frame.visStr();
    Frame &frame = frames[iFr];
    frame.name = name; frame.width = width; frame.material = material; frame.vis = vis;
  }
  ...
  for (iFr = 0; iFr<2; iFr++) {
    Frame &frame = frames[iFr]; 
    printout(DEBUG,"MPGDCylinderBarrelTracker",
	     "Frame #%d \"%s\": width = %.2f cm material = \"%s\" vis = \"%s\"",
	     iFr,frame.name.c_str(),frame.width,frame.material.c_str(),frame.vis.c_str());
  }
  ...
  if (si.size()!=1) {
    printout(ERROR,"MPGDCylinderBarrelTracker",
	     "Number of services = %d. Must be = 1",(int)si.size());
    throw runtime_error("Logics error in building modules.");
  }
  ...
  for (int iSM = 0; iSM<nStaveModels; iSM++) {
    ...
    // Start/stop frames
    double frame_dphi = zthickness/stave_rmin; //converting the thickness of the frame to angular radians.
	...
	if (sensitiveComp) {
	  printout(ERROR,"MPGDCylinderBarrelTracker",
		   "Component \"%s\": 2nd sensitive component in module \"%s\" (1st is \"%s\"). One only allowed.",
  ...
  for (int iz = 0; iz<4; iz++) {
    ...
    int iSM0 = iSMs[0]; const StaveModel &sm0 = staveModels[iSM0];
    int nphi = sm0.nphi; double offset = sm0.offset;
    double phi_incr = 2*M_PI/nphi;   // Phi increment for one module.
    // ***** LOOP OVER THE STAVES IN phi.
    int iphi; double phic; for (iphi = 0, phic = phi0; iphi<nphi;
				iphi++, phic += phi_incr, nModules++) {
      // Every other module...
