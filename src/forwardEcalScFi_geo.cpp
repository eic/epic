...
static Ref_t createDetector(Detector& desc, xml_h handle, SensitiveDetector sens) {
  ...
  Box insert((insert_dx[0] + insert_dx[1] - insert_thickness + nsgap) / 2.0,
             (insert_dy - insert_thickness) / 2.0, length / 2.);
      ...
      if (x_slice.isSensitive()) {
        ...
        for (int iy = 0; iy < ny; iy++) {
          ...
          pv = tower_vol.placeVolume(
              frow_vol, Transform3D(RotationZYX(0, 0, 0),
        ...
        for (int ix = 0; ix < nx; ix++) {
          ...
          pv =
              frow_vol.placeVolume(fcol_vol, Transform3D(RotationZYX(0, 0, 0), Position(xx, 0, 0)));
