...
#ifndef ROOT_TGDMLParseBiggerFiles
#define ROOT_TGDMLParseBiggerFiles
...
/*************************************************************************
 * TGDMLParseBiggerFiles - base class for the import of GDML to ROOT.               *
 *************************************************************************/
...
public:
  TGDMAssignmentHelper(TGDMLBaseTGDMMapHelper& baseMap, const std::string& key) {
    baseMap
        [key]; // if we do not have this key-value pair before, insert it now (with zero for pointer).
...
template <class T> class TGDMMapHelper : public TGDMLBaseTGDMMapHelper {
public:
...
class TGDMLParseBiggerFiles : public TObject {
public:
  TString fWorldName;
  ...
  ~TGDMLParseBiggerFiles() override {}
...
private:
  ...
  //'solids' section
  XMLNodePointer_t OpticalSurfaceProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                         XMLAttrPointer_t attr);
  XMLNodePointer_t BooSolid(TXMLEngine* gdml, XMLNodePointer_t node, XMLAttrPointer_t attr,
                            int num);
  ...
  XMLNodePointer_t SkinSurfaceProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                      XMLAttrPointer_t attr);
  XMLNodePointer_t BorderSurfaceProcess(TXMLEngine* gdml, XMLNodePointer_t node,
                                        XMLAttrPointer_t attr);
  ...
  ReflectionsMap
      freflectmap; //! Map containing reflection names and the Solid name ir references to
  ReflSolidMap
      freflsolidmap; //! Map containing reflection names and the TGDMLRefl for it - containing refl matrix
  ReflVolMap freflvolmap; //! Map containing reflected volume names and the solid ref for it
  FileMap
      ffilemap; //! Map containing files parsed during entire parsing, with their world volume name
  ...
  //ClassDefOverride(TGDMLParseBiggerFiles, 0) // imports GDML using DOM and binds it to ROOT
};
