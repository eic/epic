...
static Ref_t create_detector(Detector& description, xml_h e,
      ...
      for (int ix = 0; ix < nx; ix++) {
        for (int iy = 0; iy < ny; iy++) {
          ...
          for (size_t ic = 0; ic < sensVols.size(); ++ic) {
            ...
            DetElement comp_de(
                mod_de, std::string("de_") + sens_pv.volume().name(), ic + 1);
  ...
  for (xml_coll_t i(x_det, _U(layer)); i; ++i, ++l_num) {
    ...
    for (ci.reset(); ci; ++ci) {
      ...
      // auto de       = ;
      auto comp_de = module_assembly_delements[comp_assembly].clone(
          comp_assembly + std::to_string(l_num));
      if (c_pos) {
