...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens)
{
  ...
  for (xml_coll_t xc(x_det, _U(layer)); xc; ++xc) {
    ...
    PlacedVolume s_phv1 = endcapVol.placeVolume(halfdisk, Position(-disksGap / 2, 0, 0));
    s_phv1.addPhysVolID("halfdisk", 0);
    PlacedVolume s_phv2 = endcapVol.placeVolume(halfdisk, Transform3D(RotationZYX(M_PI, 0, 0), Position(+disksGap / 2, 0, 0)));
