...
static Ref_t create_detector(Detector& det, xml_h e, SensitiveDetector /* sens */)  {
  ...
  double innerD1     = IP_pipe_c.hasAttr(_Unicode(innerD1)) ?
    IP_pipe_c.attr<double>(_Unicode(innerD1)) :
    IP_pipe_c.attr<double>(_Unicode(outerD1)) - 2*thickness;
  double innerD2     = IP_pipe_c.hasAttr(_Unicode(innerD2)) ?
    IP_pipe_c.attr<double>(_Unicode(innerD2)) :
